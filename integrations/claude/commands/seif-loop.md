...
2. **Pull memory context first** (continuity, never re-derive from chat):
   `python3 ~/seif/logos/../memory/tripartite.py` is a library — in a quick Python snippet do
   `from memory.tripartite import Memory; m=Memory(); print(m.continuity_snapshot())` from `~/seif` to see
   recent episodes + reusable lessons + L1 backend. Fold prior lessons for this `task_id` into the task text.

3. **Drive the gate deterministically:**
   `cd ~/seif && python3 logos/seif_loop.py --backlog <file> --max-tasks N --budget 3 --min-accept-rate 0.34`
